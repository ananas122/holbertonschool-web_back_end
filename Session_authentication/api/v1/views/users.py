...
def view_all_users() -> str:
    """ GET /api/v1/users
    Retourne:
      - liste de tous les objets User en JSON
    """
...
def view_one_user(user_id: str = None) -> str:
    ...
    Retourne:
      - objet User en JSON
      - 404 si l'ID utilisateur n'existe pas
    """
    if user_id is None:
        abort(404)
...
def create_user() -> str:
    ...
    Retourne:
      - objet User en JSON
      - 400 si l'utilisateur ne peut pas être créé
    """
    rj = None
    error_msg = None
    try:
...
def update_user(user_id: str = None) -> str:
    ...
    Retourne:
      - objet User en JSON
      - 404 si l'ID utilisateur n'existe pas
      - 400 si l'utilisateur ne peut pas être mis à jour
    """
    if user_id is None:
        abort(404)
    user = User.get(user_id)
    ...
    try:
        ...
        rj = None
    if rj is None:
        return jsonify({'error': "Mauvais format"}), 400
